...
#include "HLTrigger/btau/src/HLTDisplacedtktkV0Filter.h"
#include "TMath.h"
...
//
HLTDisplacedtktkV0Filter::HLTDisplacedtktkV0Filter(const edm::ParameterSet& iConfig) :
  HLTFilter(iConfig),
...
HLTDisplacedtktkV0Filter::~HLTDisplacedtktkV0Filter() = default;

void HLTDisplacedtktkV0Filter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("DisplacedVertexTag",edm::InputTag("hltDisplacedtktkV0Vtx"));
  desc.add<edm::InputTag>("BeamSpotTag",edm::InputTag("hltOnlineBeamSpot"));
  desc.add<edm::InputTag>("TrackTag",edm::InputTag("hltJpsiTkAllConeTracksIter"));
  descriptions.add("hltDisplacedtktkFilter", desc);  
}

// ------------ method called once each job just before starting event loop  ------------
void HLTDisplacedtktkV0Filter::beginJob()
{

}

// ------------ method called once each job just after ending the event loop  ------------
void HLTDisplacedtktkV0Filter::endJob()
{
 	
}

// ------------ method called on each new Event  ------------
bool HLTDisplacedtktkV0Filter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  // get beam spot
  ...
  for(auto displacedVertex : displacedVertexColl){
    ...
    if(displacedVertex.tracksSize() != 2){
      edm::LogError("HLTDisplacedtktkV0Filter") << "HLTDisplacedtktkV0Filter: ERROR: the V0 vertex must have exactly two tracks by definition. It now has n tracks = "<< displacedVertex.tracksSize() << std::endl;
      return false;
    } 
    ...
    reco::TrackRef vertextkRef2 =  (*trackIt).castTo<reco::TrackRef>();
	  ...
	  if(iFoundRefs != 2){
      edm::LogError("HLTDisplacedtktkV0Filter") << "HLTDisplacedtktkV0Filter: ERROR: the V0 vertex must have exactly two tracks by definition."  << std::endl;
      return false;
    } 	
    ...
    triggered = true;
	  ...
	  filterproduct.addObject(TriggerTrack,ref1);
	
	  ref2=reco::RecoChargedCandidateRef( edm::Ref<reco::RecoChargedCandidateCollection> (trackcands,distance(trackcands->begin(),cand2 )));
	  filterproduct.addObject(TriggerTrack,ref2);
  }

  LogDebug("HLTDisplacedtktkV0Filter") << " >>>>> Result of HLTDisplacedtktkV0Filter is "<< triggered <<std::endl;
